...
func (d *CommandDispatcher) cmdRecall(ch *types.Character, args string) {
	...
	if ch.Fighting != nil {
		...
		skill := 40 // Default skill - everyone knows basic recall
...
func (d *CommandDispatcher) cmdExits(ch *types.Character, args string) {
	if ch.InRoom == nil {
	...
	d.send(ch, "Obvious exits from room:\r\n")
	...
	for dir := types.Direction(0); dir < types.DirMax; dir++ {
		...
		// Get direction name padded to consistent width
		dirName := dir.String()

		// Check if it's a closed door
		if exit.Flags.Has(types.ExitClosed) {
			d.send(ch, fmt.Sprintf("%-5s - (closed)\r\n", dirName))
			continue
		}
		...
		destName := "Unknown"
		if exit.ToRoom != nil {
			destName = exit.ToRoom.Name
		}

		d.send(ch, fmt.Sprintf("%-5s - %s\r\n", dirName, destName))
	}

...
// Supports N.mob syntax (e.g., "2.guard" for the second guard)
func FindCharInRoom(ch *types.Character, name string) *types.Character {
	...
	// Parse N.mob syntax
	count, searchName := parseNumberPrefix(name)
	...
	for _, person := range ch.InRoom.People {
		...
		if nameMatches(person.Name, searchName) || keywordsMatch(person.ShortDesc, searchName) {
			found++
			if found == count {
				return person
			}
...
// Supports N.item syntax (e.g., "2.sword" for the second sword)
func FindObjInInventory(ch *types.Character, name string) *types.Object {
	...
	found := 0
	for _, obj := range ch.Inventory {
		if nameMatches(obj.Name, searchName) || keywordsMatch(obj.ShortDesc, searchName) {
			found++
			if found == count {
				return obj
			}
...
// Supports N.item syntax (e.g., "2.corpse" for the second corpse)
...
func FindObjOnChar(ch *types.Character, name string) *types.Object {
	...
	for i := 0; i < int(types.WearLocMax); i++ {
		obj := ch.Equipment[i]
		if obj != nil && (nameMatches(obj.Name, searchName) || keywordsMatch(obj.ShortDesc, searchName)) {
			found++
			if found == count {
				return obj
			}
...
// parseNumberPrefix parses "N.item" syntax, returning the count and item name
// For example: "2.corpse" returns (2, "corpse"), "sword" returns (1, "sword")

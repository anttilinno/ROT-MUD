...
func (g *GameLoop) processConditions() {
	for _, ch := range g.Characters {
		...
		// NOTE: Hunger and thirst are disabled until food/drink shops are implemented
...
func (g *GameLoop) hitGain(ch *types.Character) int {
	...
	if ch.IsNPC() {
		...
		// NOTE: Hunger/thirst penalties disabled until food/drink shops are implemented
		// if ch.PCData != nil {

		...
		if combat.NumberBits(4) == 0 {
			...
			if len(exits) > 0 {
				...
				if exit != nil && exit.ToRoom != nil && !exit.IsClosed() {
					// Check if destination room has no_mob flag
					if exit.ToRoom.Flags.Has(types.RoomNoMob) {
						return false
					}

					// Check stay_area flag - don't leave the area
					if ch.Act.Has(types.ActStayArea) {
						// Check if destination room is in the same area
						if ch.InRoom.Area != nil && exit.ToRoom.Area != nil {
							if ch.InRoom.Area != exit.ToRoom.Area {
								// Different area, don't move
								return false
							}
						} else if ch.InRoom.Area != nil {
							// Destination has no area, check vnum range
							destVnum := exit.ToRoom.Vnum
							if destVnum < ch.InRoom.Area.MinVnum || destVnum > ch.InRoom.Area.MaxVnum {
								// Outside area vnum range, don't move
								return false
							}
						}
					}

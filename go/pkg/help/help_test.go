...
// TestCharacterCreationHelpFiles verifies that all classes and races have help entries
func TestCharacterCreationHelpFiles(t *testing.T) {
	// Load the actual help files
	sys := NewSystem()
	helpDir := "../../data/help"

	if err := sys.LoadDir(helpDir); err != nil {
		t.Fatalf("Failed to load help directory: %v", err)
	}
	...
	t.Run("Classes", func(t *testing.T) {
		...
		for _, class := range classes {
			entry := sys.Find(class)
			if entry == nil {
				missingClasses = append(missingClasses, class)
			} else {
				// Verify the entry has meaningful content
				if len(entry.Description) < 50 {
					t.Errorf("Class %q has insufficient description (len=%d)", class, len(entry.Description))
				}
			}
		}
		if len(missingClasses) > 0 {
			t.Errorf("Missing help entries for classes: %v", missingClasses)
		}
	})

	t.Run("Races", func(t *testing.T) {
		var missingRaces []string
		for _, race := range races {
			entry := sys.Find(race)
			if entry == nil {
				missingRaces = append(missingRaces, race)
			} else {
				// Verify the entry has meaningful content
				if len(entry.Description) < 50 {
					t.Errorf("Race %q has insufficient description (len=%d)", race, len(entry.Description))
				}
			}
		}
		if len(missingRaces) > 0 {
			t.Errorf("Missing help entries for races: %v", missingRaces)
		}
	})

	t.Run("GeneralTopics", func(t *testing.T) {
		// Check for general class/race overview topics
		generalTopics := []string{"classes", "races", "remort"}
		var missingTopics []string
		for _, topic := range generalTopics {
			entry := sys.Find(topic)
			if entry == nil {
				missingTopics = append(missingTopics, topic)
			}
		}
		if len(missingTopics) > 0 {
			t.Errorf("Missing general help topics: %v", missingTopics)
		}
	})

	t.Run("Skills", func(t *testing.T) {
		// All skills available during character creation (from login.go)
		skills := []string{
			// Combat skills
			"second attack",
			"third attack",
		...
		for _, skill := range skills {
			entry := sys.Find(skill)
			if entry == nil {
				missingSkills = append(missingSkills, skill)
			}
		}
		if len(missingSkills) > 0 {
			t.Errorf("Missing help entries for skills: %v", missingSkills)
		}
	})

	t.Run("SkillGroups", func(t *testing.T) {
		// All skill/spell groups available during character creation (from login.go getAvailableGroups)
		groups := []string{
			// Warrior groups
			"weaponsmaster",
			// Thief groups
			...
			"healing",
			"weather",
			// Note: attack, detection, protective, transportation are shared
		}

		var missingGroups []string
		for _, group := range groups {
			entry := sys.Find(group)
			if entry == nil {
				missingGroups = append(missingGroups, group)
			}
		}
		if len(missingGroups) > 0 {
			t.Errorf("Missing help entries for skill groups: %v", missingGroups)
		}
	})

	t.Run("Spells", func(t *testing.T) {
		// All spells available during character creation (from login.go spellLevels)
		spells := []string{
			// Damage spells
			"acid blast",
			"burning hands",
		...
		var missingSpells []string
		for _, spell := range spells {
			entry := sys.Find(spell)
			if entry == nil {
				missingSpells = append(missingSpells, spell)
			}
		}
		if len(missingSpells) > 0 {
			t.Errorf("Missing help entries for spells: %v", missingSpells)
		}
	})
}

...
func TestSkillRegistryFindByName(t *testing.T) {
	...
	// Prefix should NOT match with FindByName (exact only)
	skill = r.FindByName("par")
	if skill != nil {
		t.Error("expected nil for prefix 'par' with FindByName (exact only)")
	}

	// Prefix should NOT match multi-word
	skill = r.FindByName("shield")
	if skill != nil {
		t.Error("expected nil for prefix 'shield' with FindByName (exact only)")
	}

	// Exact match for multi-word
	skill = r.FindByName("shield block")
	if skill == nil || skill.Name != "shield block" {
		t.Error("expected to find 'shield block' by exact match")
	}
...
func TestSkillRegistryFindByPrefix(t *testing.T) {
	r := NewSkillRegistry()
	r.Register(NewSkill("dodge"))
	r.Register(NewSkill("parry"))
	r.Register(NewSkill("shield block"))

	// Exact match works with prefix too
	skill := r.FindByPrefix("dodge")
	if skill == nil || skill.Name != "dodge" {
		t.Error("expected to find 'dodge' by exact match")
	}

	// Prefix match
	skill = r.FindByPrefix("par")
	if skill == nil || skill.Name != "parry" {
		t.Error("expected to find 'parry' by prefix 'par'")
	}

	// Prefix match multi-word
	skill = r.FindByPrefix("shield")
	if skill == nil || skill.Name != "shield block" {
		t.Error("expected to find 'shield block' by prefix 'shield'")
	}

	// Not found
	skill = r.FindByPrefix("nonexistent")
	if skill != nil {
		t.Error("expected nil for nonexistent skill")
	}
}

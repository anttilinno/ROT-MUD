...
`
	...
	t.Run("Load mobile with special function", func(t *testing.T) {
		toml := `
[[mobiles]]
vnum = 3707
keywords = ["adept", "cleric"]
short_desc = "the adept of Satin"
long_desc = "An adept is here."
level = 103
sex = "female"
...
`
		...
		mob := mobs.Mobiles[0]
		if mob.Special != "spec_cast_adept" {
			t.Errorf("expected special 'spec_cast_adept', got '%s'", mob.Special)
		}
	})
}

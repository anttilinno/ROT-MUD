...
**Success Criteria** (what must be TRUE):
  ...
  9. Pantheon of 6-9 gods loads from TOML; players pick at L10+ via `pray <god>`; `sacrifice` / `tithe` / `offer` / `boon` / `atone` / `favor` / `pay_with_favor` / `bribe` commands implemented and persisted; each god has a temple shop with tiered access (open / worshipper / cleric / favored) and opposing-alignment refusal; cleric-of-this-god discount is the deepest; coin and favor are both valid currencies for temple purchases (favor-only items exist for iconic relics); tithe + decay loop creates recurring favor pressure on real-time clock; good and neutral temples sit inside cities, evil and chaotic temples are wilderness shrines outside the city gate; city guards enforce alignment thresholds (newbie grace L<5, suspicious -350 to -700, hostile <-700); MUD school graduation offers a non-binding destination choice (good / neutral / evil / chaotic) with a guard-sergeant escort that one-way-teleports new evil grads to the nearest evil shrine road; re-entry to good-lawful cities is gated through atonement, disguise/polymorph, or per-real-day-limited gate bribes
**Plans**: TBD

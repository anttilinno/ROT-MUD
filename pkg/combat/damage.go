...
// Based on ROM 2.4 formula - lose ~10% of level's XP or amount needed for level
func (c *CombatSystem) calculateXPLoss(ch *types.Character) int {
	// XP needed per level (simplified - ROM uses class-based values)
	xpPerLevel := 1000 * ch.Level

	// Lose ~10% of level's worth, but never go below 0 total
	loss := xpPerLevel / 10

	// Don't lose more than current XP
	if loss > ch.Exp {
		loss = ch.Exp
	}

	// Cap at level * 50 to be somewhat merciful
	maxLoss := ch.Level * 50
	if loss > maxLoss {
		loss = maxLoss
	}
...
func (c *CombatSystem) makeCorpse(ch *types.Character) {
